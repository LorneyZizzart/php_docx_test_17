...
class CreateText extends CreateElement
{
    ...
    public function createText()
    {
        ...
        if (!empty($args[1]['suppressAutoHyphens'])) {
            $this->generateSUPPRESSAUTOHYPHENS($args[1]['suppressAutoHyphens']);
        }
        ...
        if (!empty($args[1]['color']) || (isset($args[1]['theme']) && isset($args[1]['theme']['color']))) {
            $this->generateCOLOR($args[1]);
        }
        if (!empty($args[1]['sz'])) {
        ...
        if (!is_array($args[0])) {
            ...
            foreach ($args[0] as $texts) {
                ...
                if ($texts instanceof WordFragment) {
                    ...
                    if (empty($texts['theme']) && !empty($args[1]['theme'])) {
                        $texts['theme'] = $args[1]['theme'];
                    }
                    ...
                    if (!empty($texts['color']) || (isset($texts['theme']) && isset($texts['theme']['color']))) {
                        $this->generateCOLOR($texts);
                    }
    ...
    public function createTitle()
    {
        ...
        if (isset($args[0])) {
            ...
            if (
                ...
                if (isset($args[1]['color']) || (isset($args[1]['theme']) && isset($args[1]['theme']['color']))) {
                    $this->generateCOLOR($args[1]);
                }
                if (isset($args[1]['sz'])) {
                    $this->generateSZ($args[1]['sz']);
    ...
    {
        $colorXml = '<' . CreateElement::NAMESPACEWORD . ':color ' ;
        if (is_array($val) && isset($val['theme']) && isset($val['theme']['color'])) {
            $colorXml .= ' ' . CreateElement::NAMESPACEWORD . ':themeColor="' . $val['theme']['color'] . '" ';
        }
        if (is_array($val) && isset($val['color'])) {
            $colorXml .= ' ' . CreateElement::NAMESPACEWORD . ':val="' . $val['color']  . '" ';
        } else if (is_string($val)) {
            $colorXml .= ' ' . CreateElement::NAMESPACEWORD . ':val="' . $val  . '" ';
        }
        $colorXml .= '></' . CreateElement::NAMESPACEWORD . ':color>';

        $this->_xml = str_replace('__PHX=__GENERATERPR__', $colorXml . '__PHX=__GENERATERPR__', $this->_xml);
    }
    ...
    /**
     * Generate w:suppressAutoHyphens
     *
     * @access protected
     * @param string $val
     */
    protected function generateSUPPRESSAUTOHYPHENS($val = 'on')
    {
        $this->_xml = str_replace(
                '__PHX=__GENERATEPPR__', '<' . CreateElement::NAMESPACEWORD .
                ':suppressAutoHyphens w:val="' . $val . '"></' .
                CreateElement::NAMESPACEWORD . ':suppressAutoHyphens>__PHX=__GENERATEPPR__', $this->_xml
        );
    }

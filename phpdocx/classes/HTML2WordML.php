...
class HTML2WordML
{
    ...
    public function __construct($zipDocx)
    {
        ...
        self::$currentCustomListLvlOverride = array();
    ...
    public function render($html, $options, $docx = null)
    {
        ...
        if (isset($options['tableStyle'])) { // deprecated
        ...
        if (isset($options['paragraphStyle'])) { // deprecated
        ...
        if (isset($options['wordStyles']) && is_array($options['wordStyles'])) {
            ...
            } else { // change this when "tableStyle" and "paragraphStyle" disapears
        ...
        if (file_exists(__DIR__ . '/HTMLExtended.php') && self::$htmlExtended) {
    ...
    private function renderDompdf($html, $isFile = false, $filter = '*', $parseDivs = false, $baseURL = '', $disableWrapValue = false)
    {
        ...
        if (file_exists(__DIR__ . '/HTMLExtended.php') && $this->embedFonts) {
    ...
    private function _render($nodo, $depth = 0)
    {
        ...
        switch ($nodo['nodeName']) {
            ...
            case 'dd':
                ...
                if (file_exists(__DIR__ . '/HTMLExtended.php') && self::$htmlExtended && ($nodo['nodeName'] == 'figcaption' || $nodo['nodeName'] == 'caption') && isset($nodo['attributes']['data-label']) && $nodo['attributes']['data-label'] != '') {
            ...
            case 'ol':
                ...
                $listLevel = $this->getMaxValueList();

                if ($listLevel == 0) {
                    CreateDocx::$numOL = CreateDocx::uniqueNumberId(999, 32000);
            ...
            case 'ul':
                ...
                $this->countTags = array_count_values($this->openTags);
                $listLevel = $this->getMaxValueList();
                if ($listLevel == 0 && $nodo['nodeName'] == 'ul') {
                    CreateDocx::$numOL = CreateDocx::uniqueNumberId(999, 32000);
                    self::$orderedLists[] = CreateDocx::$numOL;
                }
                ...
                if ($this->customListStyles) {
                    ...
                    if ($listLevel == 0) {
                        ...
                        if (file_exists(__DIR__ . '/HTMLExtended.php') && self::$htmlExtended) {
                            self::$currentCustomListLvlOverride = array();
                            self::$currentCustomListLvlOverride[0] = null;
                            self::$currentListStyleLvlOverride = null;
                        }
                    } else {
                        ...
                        if (file_exists(__DIR__ . '/HTMLExtended.php') && self::$htmlExtended) {
                            ...
                            if (isset($nodo['attributes']) && isset($nodo['attributes']['class'])) {
                                ...
                                if (!is_null($currentListStyle)) {
                                    self::$currentCustomListLvlOverride[$listLevel] = CreateDocx::uniqueNumberId(999, 32766);
                                    ...
                                    array(
                                        ...
                                        'name' => $currentListStyle . '_' . self::$currentCustomListLvlOverride[$listLevel],
                                        'id' => self::$currentCustomListLvlOverride[$listLevel],
            ...
            case 'span':
                ...
                // CSS Extended
                if (file_exists(__DIR__ . '/HTMLExtended.php') && self::$htmlExtended) {
                    if (isset($nodo['properties']) && isset($nodo['properties']['data_custom']) && !empty(trim($nodo['properties']['data_custom']))) {
                        $this->rprStyle[] = html_entity_decode($properties['data_custom']);
                    }
                }
                $this->openTags[$depth] = $nodo['nodeName'];
                break;
            ...
            case '#text':
                ...
                if ($this->openSelect) {
                    if (count($this->selectOptions) < 25) {
                        ...
                        PhpdocxLogger::logger('The 25 limit of items that Word has stablished for a dropdown list has been exceeded', 'info');
                    }
            ...
            case 'br':
                $this->wordML .= $this->closePreviousTags($depth, $nodo['nodeName']);
                if ($this->openPs) {
                    if (file_exists(__DIR__ . '/HTMLExtended.php') && self::$htmlExtended && isset($nodo['attributes']['data-type'])) {
            ...
            case 'svg':
                        ...
                        if ($extension == 'svg') {
                            if (extension_loaded('imagick')) {
                                ...
                                try {
                                    $im = new Imagick();
                                    $svg = null;
                                    if (!is_file(str_replace('http://.', '', urldecode($nodo['attributes']['src'])))) {
                                    ...
                                    if (!$svg && is_file(str_replace('http://.', '', urldecode($nodo['attributes']['src'])))) {
                                        if (!is_null($this->streamContext)) {
                                            $svg = @file_get_contents(str_replace('http://.', '', urldecode($nodo['attributes']['src'])), false, $this->streamContext);
                                        } else {
                                            $svg = @file_get_contents(str_replace('http://.', '', urldecode($nodo['attributes']['src'])));
                                        }
                                    }
                                    $im->setBackgroundColor(new ImagickPixel('transparent'));
                                    $im->readImageBlob($svg);
                            ...
                            // if false try to get it using the image without parseURL
                            if (!$photo && is_file(str_replace('http://.', '', urldecode($nodo['attributes']['src'])))) {
                                if (!is_null($this->streamContext)) {
                                    $photo = @file_get_contents(str_replace('http://.', '', urldecode($nodo['attributes']['src'])), false, $this->streamContext);
                                } else {
                                    $photo = @file_get_contents(str_replace('http://.', '', urldecode($nodo['attributes']['src'])));
                                }
                            }
                        }
                    }
                ...
                if (file_exists(__DIR__ . '/HTMLExtended.php') && self::$htmlExtended) {
                    ...
                    if (isset($properties['max_width']) && preg_match('/^(-?\d*\.?\d*)(%|em|pt|px)?$/i', trim($properties['max_width']), $match)) {
                        ...
                        if ($match[2] == '%') {
                            $imageSize['width'] = $size[0] * ((float)$match[1] / 100);
                    ...
                    if (isset($properties['max_height']) && preg_match('/^(-?\d*\.?\d*)(%|em|pt|px)?$/i', trim($properties['max_height']), $match)) {
                        ...
                        if ($match[2] == '%') {
                            $imageSize['height'] = $size[1] * ((float)$match[1] / 100);
                            $applyPercentValue = true;
                        }
                    }
            ...
            default:
                ...
                if (file_exists(__DIR__ . '/HTMLExtended.php') && self::$htmlExtended) {
                    ...
                    if (array_key_exists($nodo['nodeName'], $extendedTags)) {
                        if ($extendedTags[$nodo['nodeName']] == 'addHeader' || $extendedTags[$nodo['nodeName']] == 'addFooter' || $extendedTags[$nodo['nodeName']] == 'addHeaderSection' || $extendedTags[$nodo['nodeName']] == 'addFooterSection') {
                            ...
                            if (!empty($wordFragmentHtmlContent)) {
                                ...
                                if ($extendedTags[$nodo['nodeName']] == 'addHeader' || $extendedTags[$nodo['nodeName']] == 'addHeaderSection') {
                                    ...
                                    $wordFragmentContent->embedHTML($wordFragmentHtmlContent, array('useHTMLExtended' => true, 'baseURL' => $this->baseURL));
                                    ...
                                    } else if ($extendedTags[$nodo['nodeName']] == 'addHeaderSection') {
                                        if (!isset($attributesContent['section'])) {
                                            $attributesContent['section'] = -1;
                                        }
                                        $options = array();
                                        if (isset($attributesContent['removeOthers'])) {
                                            $options['removeOthers'] = $attributesContent['removeOthers'];
                                        }

                                        $this->docx->addHeaderSection(array($attributesContent['type'] => $wordFragmentContent), $attributesContent['section'], $options);
                                    }
                                }

                                if ($extendedTags[$nodo['nodeName']] == 'addFooter' || $extendedTags[$nodo['nodeName']] == 'addFooterSection') {
                                    ...
                                    if ($extendedTags[$nodo['nodeName']] == 'addFooter') {
                                    ...
                                    } else if ($extendedTags[$nodo['nodeName']] == 'addFooterSection') {
                                        if (!isset($attributesContent['section'])) {
                                            $attributesContent['section'] = -1;
                                        }
                                        $options = array();
                                        ...
                                        $this->docx->addFooterSection(array($attributesContent['type'] => $wordFragmentContent), $attributesContent['section'], $options);
                                    }
                                }
                            }
            ...
            switch ($tag) {
                ...
                case 'tr':
                    ...
                    if (strpos($this->wordML, '#<w:gridCol/>#') !== false) {
                        $cellWidth = null;
                        // get the cell width
                        if (isset($this->tableGrid[$this->openTable][$row][$column][2]['width']) && $this->tableGrid[$this->openTable][$row][$column][2]['width'] !== null) {
                            list($cellWidth, $cellWidthType) = $this->_wordMLUnits($this->tableGrid[$this->openTable][$row][$column][2]['width']);
                        } else if (isset($this->tableGrid[$this->openTable][$row][$row][2]['width']) && $this->tableGrid[$this->openTable][$row][$row][2]['width'] !== null) {
                ...
                case 'ul':
                    if (file_exists(__DIR__ . '/HTMLExtended.php') && self::$htmlExtended) {
                        $listLevel = $this->getMaxValueList(1);
                        self::$currentCustomListLvlOverride[$listLevel] = null;
                        self::$currentListStyleLvlOverride = null;
                    }
                    break;
                default:
    ...
    private function generatePPr($properties, $level = '', $attributes = array(), $nodeName = false)
    {
        ...
        if (!$this->strictWordStyles) {
            ...
            foreach (self::$borders as $key => $value) {
                if (isset($properties['border_' . $value . '_style']) && $properties['border_' . $value . '_style'] != 'none') {
                    ...
                    if (file_exists(__DIR__ . '/HTMLExtended.php') && self::$htmlExtended && isset($properties['border_spacing']) && $properties['border_spacing'] != '') {
        ...
        if (!$this->strictWordStyles) {
            ...
            if (isset($properties['text_align'])) {
                if (isset(self::$text_align[$properties['text_align']])) {
        ...
        if (file_exists(__DIR__ . '/HTMLExtended.php') && self::$htmlExtended) {
            ...
            if (isset($properties['data_custom']) && !empty(trim($properties['data_custom']))) {
                $stringPPr .= html_entity_decode($properties['data_custom']);
            }
    ...
    private function generateRPr($properties, $level = '', $attributes = array(), $nodeName = false)
    {
        ...
        if (!$this->strictWordStyles) {
            if ($this->addDefaultStyles) {
                if (isset($properties['font_family']) && $properties['font_family'] != 'serif') {
                    if (!isset($properties['data_font_family']) || (isset($properties['data_font_family']) && $properties['data_font_family'] != 'initial')) {
                        $arrayCSSFonts = explode(',', $properties['font_family']);
                        $font = trim($arrayCSSFonts[0]);
                        $font = str_replace(array('"', "'"), '', $font);
        ...
        if (!$this->strictWordStyles) {
            if ($this->addDefaultStyles) {
                if (isset($properties['color']) && $properties['color'] != '' && is_array($properties['color'])) {
                    if (!isset($properties['data_color']) || (isset($properties['data_color']) && $properties['data_color'] != 'initial')) {
                        $color = $properties['color'];
                        $color = $this->wordMLColor($color);
        ...
        // HTML Extended
        if (file_exists(__DIR__ . '/HTMLExtended.php') && self::$htmlExtended && $this->rprStyle != null && is_array($this->rprStyle) && count($this->rprStyle) > 0) {
    ...
    private function generateTblPr($properties, $attributes)
    {
        ...
        if (file_exists(__DIR__ . '/HTMLExtended.php') && self::$htmlExtended) {
            if (isset($properties['data_custom']) && !empty(trim($properties['data_custom']))) {
                $stringTblPr .= html_entity_decode($properties['data_custom']);
            }
        }
    ...
    private function generateTrPr($properties, $attributes)
    {
        ...
        if (file_exists(__DIR__ . '/HTMLExtended.php') && self::$htmlExtended) {
            if (isset($properties['data_custom']) && !empty(trim($properties['data_custom']))) {
                $stringTrPr .= html_entity_decode($properties['data_custom']);
            }
        }
    ...
    private function generateTcPr($properties, $attributes, $colspan, $rowspan, $firstRow)
    {
        ...
        if (file_exists(__DIR__ . '/HTMLExtended.php') && self::$htmlExtended) {
            if (isset($properties['data_custom']) && !empty(trim($properties['data_custom']))) {
                $stringTcPr .= html_entity_decode($properties['data_custom']);
            }
        }
    ...
    private function generateListPr($properties, $level = '', $attributes = array(), $nodeName = false)
    {
        ...
        if (isset($properties['list_style_type']) && $properties['list_style_type'] == 'none') {
            ...
            $this->countTags = array_count_values($this->openTags);
            if (file_exists(__DIR__ . '/HTMLExtended.php') && self::$htmlExtended && isset($attributes['data-depth']) && $attributes['data-depth'] != '') {
                $stringListPr .= $attributes['data-depth'];
            } else {
                $stringListPr .= $this->getMaxValueList(1);
            }
            $stringListPr .= '"/><w:numId w:val="';
            if (file_exists(__DIR__ . '/HTMLExtended.php') && self::$htmlExtended) {
                $maxValue = $this->getMaxValueList(1);
                if ($maxValue == 0) {
                    self::$currentCustomListLvlOverride = array();
                    self::$currentCustomListLvlOverride[0] = null;
                }
            ...
            if (file_exists(__DIR__ . '/HTMLExtended.php') && self::$htmlExtended) {
                $listLevel = $this->getMaxValueList(1);
                if (!empty(self::$currentCustomListLvlOverride[$listLevel]) && isset(self::$currentCustomListLvlOverride[$listLevel])) {
                    $stringListPr .= self::$currentCustomListLvlOverride[$listLevel];
        ...
        if (file_exists(__DIR__ . '/HTMLExtended.php') && self::$htmlExtended) {
            if (isset($properties['data_custom']) && !empty(trim($properties['data_custom']))) {
                $stringListPr .= html_entity_decode($properties['data_custom']);
            }
        }
    ...
    /**
     * Get max value for lists
     * @param int $decrement
     *
     * @return int
     */
    private function getMaxValueList($decrement = 0) {
        $maxValue = 0;
        if (isset($this->countTags['ul']) || isset($this->countTags['ol'])) {
            if (isset($this->countTags['ul'])) {
                $maxValue += $this->countTags['ul'];
            }
            if (isset($this->countTags['ol'])) {
                $maxValue += $this->countTags['ol'];
            }
            $maxValue = $maxValue - $decrement;
        }
        if ($maxValue < 0) {
            $maxValue = 0;
        }
    ...
    private function generateImageRPr($properties, $height = 0)
    {
        ...
        if (preg_match('/^(-?\d*\.?\d*)(%|em|pt|px)?$/i', trim($properties['vertical_align']), $match)) {
            ...
            switch ($match[2]) {
                case '%':
                    $position = ceil(2 * (float)$match[1] * $ptHeight / 100);
                case 'em':
                    $position = ceil(2 * (float)$match[1] * $properties['font_size']);
                    break;
                case 'pt':
                    $position = ceil(2 * (float)$match[1]);
                    break;
                case 'px': //a pixel is around 3/4 of a point
                default: //if no unit we asume is given in pixels
                    $position = ceil(2 * (float)$match[1] * 0.75);
            }
        ...
        foreach ($contents as $content) {
            if (isset($content->tagName) && $content->tagName == $textDocumentScope) {
                ...
                foreach ($contentText as $contentTextElement)  {
                    ...
                    $currentLinkImages = self::$linkImages;
                    $currentLinkTargets = self::$linkTargets;
                    $wordFragmentTextContent->embedHTML($wordFragmentHtmlContent, array('useHTMLExtended' => true, 'baseURL' => $this->baseURL));
                    self::$linkImages = array_merge(self::$linkImages, $currentLinkImages);
                    self::$linkTargets = array_merge(self::$linkTargets, $currentLinkTargets);
                    ...
                    if ($contentTextElement->hasAttribute('data-parentid')) {
                        $newContentToBeAdded['parentId'] = $contentTextElement->getAttribute('data-parentid');
                    }
     ...
     * @param mixed $font_size Font size, if applicable
     * @return mixed
     */
    private function _rprPosition($valign, $font_size)
    {
        $measureUnit = substr($valign, -2);
        $quantity = (int) substr($valign, 0, -2);
        if ($valign == 'middle') {
            $measureUnit = 'em';
            $quantity = -0.5;
        }

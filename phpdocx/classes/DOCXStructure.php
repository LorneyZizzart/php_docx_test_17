...
class DOCXStructure
{
    ...
    public function saveDocx($path, $forceFile = false) {
        ...
        if (substr($path, -5) !== '.docx' && substr($path, -5) !== '.docm' && substr($path, -5) !== '.dotx' && substr($path, -5) !== '.dotm') {
            $path .= '.docx';
        }

        // return the structure object instead of creating the file
        if (file_exists(__DIR__ . '/ZipStream.php') && CreateDocx::$returnDocxStructure == true) {
            return $this;
        }

        // check if stream mode is true
        if (file_exists(__DIR__ . '/ZipStream.php') && CreateDocx::$streamMode === true && $forceFile === false) {

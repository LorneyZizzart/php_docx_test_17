...
class TemplateProcessing
{
    ...
    public function replaceMultiTextVariable($vars)
    {
        ...
        $this->_parsedXMLDOM['word/document.xml'] = $xmlUtilities->generateSimpleXmlElement($docXML);
    }
    ...
    public function processTemplate($template, $options = array())
    {
        ...
        $this->_templateRels = $xmlUtilities->generateSimpleXmlElement($this->_templateContent['word/_rels/document.xml.rels']);
        ...
        foreach ($this->_parsedXML as $key => $value) {
            $this->_parsedXMLDOM[$key] = $xmlUtilities->generateSimpleXmlElement($value);
        }
    }
    ...
    private function repairSingleVariable($var, $content)
    {
        ...
        if (isset($preserve) && $preserve) {
            ...
            $xmlUtilities = new XmlUtilities();
            $docDOM = $xmlUtilities->generateDomDocument($content);
    ...
    {
        $xmlUtilities = new XmlUtilities();
        $relsDOM = $xmlUtilities->generateDomDocument($this->_parsedContent['word/_rels/document.xml.rels']);
        $relsXPath = new DOMXPath($relsDOM);
        $relsXPath->registerNamespace('rel', 'http://schemas.openxmlformats.org/package/2006/relationships');

        $domDocument = $xmlUtilities->generateDomDocument($this->_parsedContent['word/document.xml']);
        ...
        foreach ($chartData as $idChart => $data) {
            ...
            $domChart = $xmlUtilities->generateDomDocument($this->_parsedContent[$chartName]);
            ...
            $charRelsDOM = $xmlUtilities->generateSimpleXmlElement($this->_parsedContent['word/charts/_rels/' . $shortChartName . '.xml.rels']);

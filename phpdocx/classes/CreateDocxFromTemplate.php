...
class CreateDocxFromTemplate extends CreateDocx
{
     ...
     * @var string
     */
    public static $regExprVariableParse = '[^\s](.+)[^\s]';
     ...
     * @access public
     * @throws Exception empty template, error opening the template
     */
    ...
    public function setTemplateSymbol($templateSymbolStart = '$', $templateSymbolEnd = null)
    {
        ...
        if (function_exists('mb_strlen')) {
            $lengthTemplateSymbolStart = mb_strlen($templateSymbolStart);
        } else {
            $lengthTemplateSymbolStart = strlen($templateSymbolStart);
        }

        if ($templateSymbolStart == $templateSymbolEnd) {
            $templateSymbolEnd = null;
        }

        // symbols with more than one character (##, ||) must be handled as distinct symbols to work with them correctly to wrap variables
        if (is_null($templateSymbolEnd) && $lengthTemplateSymbolStart > 1) {
    ...
    public function getTemplateVariables($target = 'all', $prefixes = array(), $variables = array())
    {
        ...
        if (!$this->_parseMode) {
            // default mode
            if (function_exists('mb_strlen')) {
                $lengthTemplateSymbolStart = mb_strlen($this->_templateSymbolStart);
            } else {
                $lengthTemplateSymbolStart = strlen($this->_templateSymbolStart);
            }

            if ($target == 'document') {
                if ($this->_templateSymbolStart == $this->_templateSymbolEnd && $lengthTemplateSymbolStart == 1) {
    ...
    {
        if (!file_exists(__DIR__ . '/DOCXPathStyles.php')) {
            PhpdocxLogger::logger('This method is not available for your license.', 'fatal');
        }
    ...
    public function removeTemplateVariable($variableName, $type = 'block', $target = 'document')
    {
        if ($type == 'inline') {
            ...
            if ($target == 'document') {
                ...
                $this->_wordFootnotesT = $this->xmlUtilities->generateDomDocument($dom);
            } else if ($target == 'endnote') {
                $dom = $this->removeVariableBlock($variableName, $this->_wordEndnotesT->saveXML());
                $this->_wordEndnotesT = $this->xmlUtilities->generateDomDocument($dom);
            } else if ($target == 'comment') {
                $dom = $this->removeVariableBlock($variableName, $this->_wordCommentsT->saveXML());
                $this->_wordCommentsT = $this->xmlUtilities->generateDomDocument($dom);
            } else if ($target == 'header') {
                $xpathHeaders = simplexml_import_dom($this->_contentTypeT);
                $xpathHeaders->registerXPathNamespace('ns', 'http://schemas.openxmlformats.org/package/2006/content-types');
                $xpathHeadersResults = $xpathHeaders->xpath('ns:Override[@ContentType="application/vnd.openxmlformats-officedocument.wordprocessingml.header+xml"]');
     ...
     * 'wordFragmentOptions' (array) options for WordFragment values. Premium licenses
    ...
    public function replaceListVariable($variable, $listValues, $options = array())
    {
        ...
        if (!isset($options['wordFragmentOptions'])) {
            $options['wordFragmentOptions'] = array();
        }
        ...
        if ($target == 'document') {
            ...
            $dom = $this->xmlUtilities->generateSimpleXmlElement($loadContent);
            ...
            if (count($wordFragmentsValues) > 0) {
                $wordFragmentOptions = array_merge($options['wordFragmentOptions'], array('type' => $type));
                $this->replaceVariableByWordFragment($wordFragmentsValues, $wordFragmentOptions);
            }
            ...
            foreach ($xpathHeadersResults as $headersResults) {
                ...
                if (!empty($loadContent)) {
                    ...
                    if (isset($options['parseLineBreaks']) && $options['parseLineBreaks']) {
                        ...
                        $dom = $this->xmlUtilities->generateSimpleXmlElement($stringDoc);
                    }
            ...
            // replace existing WordFragment placeholders
            if (count($wordFragmentsValues) > 0) {
                $wordFragmentOptions = array_merge($options['wordFragmentOptions'], array('type' => $type, 'target' => 'header'));
                $this->replaceVariableByWordFragment($wordFragmentsValues, $wordFragmentOptions);
            }
            ...
            // replace existing WordFragment placeholders
            if (count($wordFragmentsValues) > 0) {
                $wordFragmentOptions = array_merge($options['wordFragmentOptions'], array('type' => $type, 'target' => 'footer'));
                $this->replaceVariableByWordFragment($wordFragmentsValues, $wordFragmentOptions);
            }
        }
    }
     ...
     * 'width' (mixed) the value in cm (float) or 'auto' (use image size) or 'fit' (fit the image in the placeholder image size). 0 to not change the previous size
     * 'height' (mixed) the value in cm (float) or 'auto' (use image size) or 'fit' (fit the image in the placeholder image size). 0 to not change the previous size
     * 'dpi' (int) dots per inch. This parameter is only taken into account if width or height are set to auto or fit
     ...
     * 'hyperlink' (string) if set, adds the image hyperlink
     ...
     * 'stylesReplacementType' (string) usePlaceholderStyles (keep placeholder styles, styles from the imported HTML are ignored), mixPlaceholderStyles (mix placeholder styles, placeholder styles overwrite HTML styles with the same name). Applies to the following styles: pPr, rPr. Premium licenses
     * 'stylesReplacementTypeIgnore' (array) styles to be ignored from the imported HTML. Use with mixPlaceholderStyles. Premium licenses
     * 'stylesReplacementTypeOverwrite' (bool) if true, overwrite the placeholder styles don't set in stylesReplacementTypeIgnore. Use with mixPlaceholderStyles. Default as false. Premium licenses
    ...
    public function replaceVariableByText($variables, $options = array())
    {
        ...
        if (isset($options['raw']) && $options['raw'] === true) {
            foreach ($variables as $keyVariable => $valueVariable) {
                $valueVariable = $this->parseAndCleanTextString($valueVariable);
                if ($target == 'document') {
                    ...
                    $this->_wordFootnotesT = $this->xmlUtilities->generateDomDocument($newFootnotesTXML);
                } else if ($target == 'endnote') {
                    ...
                    $this->_wordEndnotesT = $this->xmlUtilities->generateDomDocument($newEndnotesTXML);
                } else if ($target == 'comment') {
                    ...
                    $this->_wordCommentsT = $this->xmlUtilities->generateDomDocument($newCommentsTXML);
                } else if ($target == 'header') {
                    $xpathHeaders = simplexml_import_dom($this->_contentTypeT);
                    $xpathHeaders->registerXPathNamespace('ns', 'http://schemas.openxmlformats.org/package/2006/content-types');
                    $xpathHeadersResults = $xpathHeaders->xpath('ns:Override[@ContentType="application/vnd.openxmlformats-officedocument.wordprocessingml.header+xml"]');
     ...
     * 'firstMatch' (bool) if true it only replaces the first variable match. Default is set to false.
     * 'stylesReplacementType' (string) usePlaceholderStyles (keep placeholder styles, styles from the imported HTML are ignored), mixPlaceholderStyles (mix placeholder styles, placeholder styles overwrite HTML styles with the same name). Applies to the following styles: pPr, rPr. Premium licenses
     * 'stylesReplacementTypeIgnore' (array) styles to be ignored from the imported WordFragment. Use with mixPlaceholderStyles. Premium licenses
     * 'stylesReplacementTypeOverwrite' (bool) if true, overwrite the placeholder styles don't set in stylesReplacementTypeIgnore. Use with mixPlaceholderStyles. Default as false. Premium licenses
    ...
    public function replaceVariableByWordFragment($variables, $options = array())
    {
        ...
        if ($target == 'document') {
            ...
            $this->_wordFootnotesT = $this->xmlUtilities->generateDomDocument($stringDoc);
        } else if ($target == 'endnote') {
            $stringDoc = $this->variable4WordFragment($variables, $type, $this->_wordEndnotesT->saveXML(), $firstMatch, $options);
            $this->_wordEndnotesT = $this->xmlUtilities->generateDomDocument($stringDoc);
        } else if ($target == 'comment') {
            $stringDoc = $this->variable4WordFragment($variables, $type, $this->_wordCommentsT->saveXML(), $firstMatch, $options);
            $this->_wordCommentsT = $this->xmlUtilities->generateDomDocument($stringDoc);
        } else if ($target == 'header') {
            $xpathHeaders = simplexml_import_dom($this->_contentTypeT);
            $xpathHeaders->registerXPathNamespace('ns', 'http://schemas.openxmlformats.org/package/2006/content-types');
            $xpathHeadersResults = $xpathHeaders->xpath('ns:Override[@ContentType="application/vnd.openxmlformats-officedocument.wordprocessingml.header+xml"]');
     ...
     *      'firstMatch' (bool) if true it only replaces the first variable match. Default is set to false
     */
    public function tickCheckboxes($variables, $options = array())
    {
        if (isset($options['firstMatch'])) {
            $firstMatch = $options['firstMatch'];
        } else {
    ...
    private function getDpiPng($filename)
    {
        ...
        while (!feof($a)) {
            $contentFread = fread($a, 1);
            if (empty($contentFread)) {
                continue;
            }
            array_push($buf, ord($contentFread));
            if (count($buf) > 13) {
                array_shift($buf);
            }
            if (count($buf) < 13) {
    ...
    private function Image4Image($variable, $src, $loadContent, $options = array(), $rels = 'document')
    {
        ...
        if (isset($options['dpi'])) {
            ...
            if ((isset($options['width']) && ($options['width'] == 'auto' || $options['width'] == 'fit')) ||
                    (isset($options['height']) && ($options['height'] == 'auto' || $options['height'] == 'fit'))) {
        ...
        if (isset($options['width']) && $options['width'] == 'fit' && !isset($options['height'])) {
            $options['height'] = 'auto';
        }
        if (isset($options['height']) && $options['height'] == 'fit' && !isset($options['width'])) {
            $options['width'] = 'auto';
        }
        $width = 0;
        $height = 0;
        ...
        if (isset($options['height']) && $options['height'] != 'auto' && $options['height'] != 'fit') {
        ...
        $imageCounter = 0;
        //create a new Id
        $id = uniqid((string)mt_rand(99,9999999));
        $ind = 'rId' . $id;
        $relsCounter = 0;
        if (isset($options['hyperlink'])) {
            $options['hyperlink'] = $this->parseAndCleanTextString($options['hyperlink']);
        }
        for ($i = 0; $i < $domImages->length; $i++) {
            if ($imageCounter == 0) {
                ...
                if ($rels == 'document') {
                    ...
                    if (isset($options['hyperlink'])) {
                        if (strpos($options['hyperlink'], '#') === 0) {
                            // bookmark
                            $this->generateRELATIONSHIP('rId' . $id . 'link', 'hyperlink', $options['hyperlink']);
                        } else {
                            // external hyperlink
                            $this->generateRELATIONSHIP('rId' . $id . 'link', 'hyperlink', $options['hyperlink'], 'TargetMode="External"');
                        }
                    }
                    ...
                    if (isset($options['hyperlink'])) {
                        if (strpos($options['hyperlink'], '#') === 0) {
                            // bookmark
                            self::$_relsNotesLink[$rels][] = array('rId' => 'rId' . $id . 'link', 'url' => $options['hyperlink']);
                        } else {
                            // external hyperlink
                            self::$_relsNotesLink[$rels][] = array('rId' => 'rId' . $id . 'link', 'url' => $options['hyperlink'], 'TargetMode' => 'External');
                        }
                    }
                } else {
                    ...
                    if (
                        strpos($relsXML, $relationship) === false
                    ) {
                    ...
                    if (isset($options['hyperlink'])) {
                        if (strpos($options['hyperlink'], '#') === 0) {
                            // bookmark
                            $relationship = '<Relationship Target="' . $options['hyperlink'] . '" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/hyperlink" Id="rId' . $id . 'link"/>';
                        } else {
                            // external hyperlink
                            $relationship = '<Relationship Target="' . $options['hyperlink'] . '" TargetMode="External" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/hyperlink" Id="rId' . $id . 'link"/>';
                        }
                        if (
                            strpos($relsXML, $relationship) === false
                        ) {
                ...
                if ((isset($options['width']) && $options['width'] == 'fit') || (isset($options['height']) && $options['height'] == 'fit')) {
                    // fit mode
                    $cxFit = $domImages->item($i)->parentNode
                            ->getElementsByTagNameNS('http://schemas.openxmlformats.org/drawingml/2006/wordprocessingDrawing', 'extent')
                            ->item(0)->getAttribute('cx');
                    $cyFit = $domImages->item($i)->parentNode
                            ->getElementsByTagNameNS('http://schemas.openxmlformats.org/drawingml/2006/wordprocessingDrawing', 'extent')
                            ->item(0)->getAttribute('cy');
                    if ($cx == 0) {
                        $cx = $cxFit;
                    }
                    if ($cy == 0) {
                ...
                if (isset($options['hyperlink'])) {
                    // w:docPr node
                    $hlinkClinkDocPrNodes = $domImages->item($i)->getElementsByTagNameNS('http://schemas.openxmlformats.org/drawingml/2006/main', 'hlinkClick');
                    if ($hlinkClinkDocPrNodes->length > 0) {
                        $hlinkClinkDocPrNodes->item(0)->setAttribute('r:id', 'rId' . $id . 'link');
                    } else {
                        $hlinkClickElement = $domImages->item($i)->ownerDocument->createElementNS('http://schemas.openxmlformats.org/drawingml/2006/main', 'a:hlinkClick');
                        $hlinkClickElement->setAttribute('r:id', 'rId' . $id . 'link');
                        $hlinkClickElement->setAttribute('xmlns:a', 'http://schemas.openxmlformats.org/drawingml/2006/main');
                        $hlinkClickElement->setAttribute('xmlns:r', 'http://schemas.openxmlformats.org/officeDocument/2006/relationships');
                        $domImages->item($i)->appendChild($hlinkClickElement);
                    }

                    // cNvPr node
                    $cNvPrNodes = $domImages->item($i)->parentNode->getElementsByTagNameNS('http://schemas.openxmlformats.org/drawingml/2006/picture', 'cNvPr');
                    if ($cNvPrNodes->length > 0) {
                        $hlinkClickCnvPrNodes = $cNvPrNodes->item(0)->getElementsByTagNameNS('http://schemas.openxmlformats.org/drawingml/2006/main', 'hlinkClick');
                        if ($hlinkClickCnvPrNodes->length > 0) {
                            $hlinkClickCnvPrNodes->item(0)->setAttribute('r:id', 'rId' . $id . 'link');
                        } else {
                            $hlinkClickElement = $domImages->item($i)->ownerDocument->createElementNS('http://schemas.openxmlformats.org/drawingml/2006/main', 'a:hlinkClick');
                            $hlinkClickElement->setAttribute('r:id', 'rId' . $id . 'link');
                            $hlinkClickElement->setAttribute('xmlns:a', 'http://schemas.openxmlformats.org/drawingml/2006/main');
                            $hlinkClickElement->setAttribute('xmlns:r', 'http://schemas.openxmlformats.org/officeDocument/2006/relationships');
                            $cNvPrNodes->item(0)->appendChild($hlinkClickElement);
                        }
                    }
                }
            }
    ...
    private function repairVariables($variables, $content)
    {
        if (!$this->_parseMode) {
            ...
            foreach ($nodesP as $nodeP) {
                ...
                $pVariablesReplacements = array();
                foreach ($variables as $var => $value) {
                    if (stripos($contentNodePCleaned, $this->_templateSymbolStart . $var . $this->_templateSymbolEnd) !== false) {
                        if (function_exists('mb_strlen')) {
                            $charsCount = mb_strlen($contentNodeP);
                        } else {
                            $charsCount = strlen($contentNodeP);
                        }
                        // iterate $contentNodeP to get the XML of $var
                        $tempSearch = '';
                        $xmlContent = '';
                        $inTag = false;
                        for ($i = 0; $i < $charsCount; $i++) {
                            if (function_exists('mb_strlen')) {
                                $tempSearchLength = mb_strlen($tempSearch);
                            } else {
                                $tempSearchLength = strlen($tempSearch);
                            }
                            if ($tempSearchLength == 0) {
                                $xmlContent = '';
                            }
                            ...
                            if (!$inTag) {
                                $tempSearch .= $char;
                                $tempSearch = ltrim($tempSearch);
                                // check if the variable has been found
                                if (!empty($tempSearch) && strstr($this->_templateSymbolStart . $var . $this->_templateSymbolEnd, $tempSearch) !== false) {
                                    // the variable has been found found, but it's not already cleaned
                                    if ($tempSearch == $this->_templateSymbolStart . $var . $this->_templateSymbolEnd && stripos($xmlContent, $this->_templateSymbolStart . $var . $this->_templateSymbolEnd) === false) {
                                        // keep replacements to be done in the paragraph
                                        $pVariablesReplacements[] = array(
                                            'xml' => $xmlContent,
                                            'cleaned' => strip_tags($xmlContent),
                                        );
                                        // reset the contents, the same variable may exists in the same paragraph. Keep the last char to avoid ignoring duplicated strings (such as when if using $)
                                        $tempSearch = $char;
                                        $xmlContent = $char;
                                    }
                                } else {
                                    // the variable has not been found, reset the contents. Keep the last char to avoid ignoring duplicated strings (such as $$ when using $). Ignore blank spaces at the beginning of the search
                                    if ($char == ' ') {
                                        $tempSearch = '';
                                        $xmlContent = '';
                                    } else {
                ...
                // instead of doing a global replacement in the content, do the replacements in the paragraph content and then replace the whole paragraph in the global content
                if (count($pVariablesReplacements) > 0) {
                    $contentNodePReplaced = $contentNodeP;
                    foreach ($pVariablesReplacements as $pVariableReplacement) {
                        $contentNodePReplaced = str_replace($pVariableReplacement['xml'], $pVariableReplacement['cleaned'], $contentNodePReplaced);
                    }
                    $content = str_replace($contentNodeP, $contentNodePReplaced, $content);
                }
            }
    ...
    private function singleVariable4WordFragment($var, $val, $type, $loadContent, $firstMatch, $options = array())
    {
        ...
        if (file_exists(__DIR__ . '/WordFragmentExtended.php')) {
        ...
        foreach ($docNodes as $node) {
            ...
            if (strpos($cleanNodeText, $searchVariable) !== false) {
                if ($type == 'block') {
                    ...
                    $node->parentNode->insertBefore($cursorNode, $node);
                    $node->parentNode->removeChild($node);
                } else if ($type == 'inline-block' && file_exists(__DIR__ . '/WordFragmentExtended.php')) {
                    $textNode = $node->ownerDocument->saveXML($node);

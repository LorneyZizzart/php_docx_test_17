...
class CreateTable extends CreateElement
{
    ...
    public function createTable()
    {
        ...
        if (is_array($args[0])) {
            ...
            if (CreateDocx::getChunkMode()) {
                $tableXml = $this->_xml;
                $rowsXml = '';
            }
            foreach ($tableData as $row) {
                if (CreateDocx::getChunkMode()) {
                    $this->_xml = '__PHX=__GENERATETBL__';
                }
                $this->generateTR();
                $this->generateTRPR();
            ...
            if (CreateDocx::getChunkMode()) {
                $this->_xml = $tableXml;
                $this->_xml = str_replace('__PHX=__GENERATETBL__', $rowsXml, $this->_xml);
            }
        }
    }
    ...
    protected function generateTBLFLOAT($float)
    {
        ...
        $horzAnchor = 'margin';
        if (isset($float['horzAnchor'])) {
            $horzAnchor = $float['horzAnchor'];
        }
        $vertAnchor = 'text';
        if (isset($float['vertAnchor'])) {
            $vertAnchor = $float['vertAnchor'];
        }
        $tblpXSpec = $align;
        if (isset($float['tblpXSpec'])) {
            $tblpXSpec = $float['tblpXSpec'];
        }
        $tblpYSpec = 'inside';
        if (isset($float['tblpYSpec'])) {
            $tblpYSpec = $float['tblpYSpec'];
        }
        ...
        $xml .= 'w:vertAnchor="' . $vertAnchor . '" w:horzAnchor ="' . $horzAnchor . '" ';
        $xml .= 'w:tblpXSpec ="' . $tblpXSpec . '" w:tblpYSpec="' . $tblpYSpec . '" />';

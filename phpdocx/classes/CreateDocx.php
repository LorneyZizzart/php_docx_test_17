...
require_once __DIR__ . '/Phpdocx_config.php';

class CreateDocx extends CreateDocument
{
    const PHPDOCX_VERSION = '17.5';
    ...
    public function __construct($baseTemplatePath = PHPDOCX_BASE_TEMPLATE, $docxTemplatePath = '')
    {
        ...
        if (file_exists(__DIR__ . '/DOCXStructureTemplate.php')) {
        ...
        // people
        if (file_exists(__DIR__ . '/Tracking.php')) {
            if ($this->getFromZip('word/people.xml') === false) {
                $this->_wordDocumentPeople = $this->xmlUtilities->generateDomDocument(OOXMLResources::$peopleXML);
                // include the corresponding relationship and Override
                $this->generateRELATIONSHIP(
        ...
        if (isset($this->_phpdocxconfig['settings']['stream']) &&
            ...
            file_exists(__DIR__ . '/ZipStream.php')) {
     ...
     * Getter chunk mode
     */
    public static function getChunkMode() {
        if (!file_exists(__DIR__ . '/ZipStream.php')) {
            self::$chunkMode = false;
        }

        return self::$chunkMode;
    }

    /**
     * Getter stream mode
     */
    public static function getStreamMode() {
     ...
     * Keys and values:
     *     'type' (string) can be * (all, default value), break, chart, endnote (content reference), footnote (content reference), image, list, math, paragraph (also for bookmarks, links and lists), section, shape, table, table-row, table-cell, table-cell-paragraph
     *     'contains' (string) for list, paragraph (text, bookmark, link), shape
     *     'occurrence' (int) exact occurrence, (array) occurrences, (string) range of contents (e.g.: 2..9, 2.., ..9) or first() or last(), if empty iterate all elements
     *     'attributes' (array)
    ...
    /**
     * Adds a barcode field. Compatible from MS Word 2013
     *
     * @access public
     * @param string $content link, text
     * @param string $type QR, CODE128, CODE39, JPPOST, EAN8|EAN13, JAN8|JAN13, UPCA|UPCE, NW7, ITF14, CASE
     * @param array $switchers Some switches are barcode type specific
     ...
     * @param array $optionsParagraph @see addText
     */
    public function addBarcode($content, $type, $switchers = array(), $optionsParagraph = array())
    {
        if ($type == 'QR' && count($switchers) == 0) {
            $switchers[] = '\q 3';
        }

        $switchersContent = implode(' ', $switchers);

        $paragraphContainer = new WordFragment();
        $paragraphContainer->addText('', $optionsParagraph);
        $paragraphContainer = preg_replace('/__PHX=__[A-Z]+__/', '', $paragraphContainer);
        $url = 'DisplayBarcode' . ' "' . $content . '" ' . $type . ' ' . $switchersContent;
        $startNodes = '<w:r><w:fldChar w:fldCharType="begin" /></w:r><w:r><w:instrText xml:space="preserve">' . $url . '</w:instrText></w:r><w:r><w:fldChar w:fldCharType="separate" /></w:r>';
        if (strstr($paragraphContainer, '</w:pPr>')) {
            $paragraphContainer = preg_replace('/<\/w:pPr>/', '</w:pPr>' . $startNodes, $paragraphContainer);
        } else {
            $paragraphContainer = preg_replace('/<w:p>/', '<w:p>' . $startNodes, $paragraphContainer);
        }
        $endNode = '<w:r><w:fldChar w:fldCharType="end" /></w:r>';
        $paragraphContainer = preg_replace('/<\/w:p>/', $endNode . '</w:p>', $paragraphContainer);

        PhpdocxLogger::logger('Add barcode to word document.', 'info');

        $contentElement = (string)$paragraphContainer;
    ...
    {
        if (!file_exists(__DIR__ . '/HTMLExtended.php')) {
            PhpdocxLogger::logger('This method is not available for your license.', 'fatal');
        }
    ...
    {
        if (!file_exists(__DIR__ . '/CreateBibliography.php')) {
            PhpdocxLogger::logger('This method is not available for your license.', 'fatal');
        }
     ...
     *          'showSeries' (int): 0, 1
     *          'showValue' (int): 0, 1
     *      'serMarkerOptions' (array) marker options (line charts)
     *          'borderColor' (string) FF0000...
     *          'color' (string) FF0000...
     *          'size' (int) 1 to 73
     *          'symbol' (string) none, dot, plus, square, star, triangle, x, diamond, circle, dash
     ...
     *  'splitPos' split position, int or float
     ...
     *
     * @throws Exception no data, legend or type values, externalXLSX values are not valid, missing legends
    ...
    {
        if (!file_exists(__DIR__ . '/CreateCitation.php')) {
            PhpdocxLogger::logger('This method is not available for your license.', 'fatal');
        }
     ...
     * 'type' (string) indented (default), run-in
     * @param array $legend for the available options @see addText
     * @throws Exception method not available
     */
    public function addIndex($options = array(), $legend = array())
    {
        if (!file_exists(__DIR__ . '/CreateIndex.php')) {
            PhpdocxLogger::logger('This method is not available for your license.', 'fatal');
        }
        ...
        $legend = self::setRTLOptions($legend);
        if (!isset($legend['text']) || empty($legend['text'])) {
            $legend['text'] = 'No index entries found.';
        }
        $legendOptions = $legend;
        unset($legendOptions['text']);
        $class = get_class($this);
        $legendData = new WordFragment();
        $legendData->addText(array($legend), $legendOptions);
        $index = CreateIndex::getInstance();
        $index->createIndex($options, $legendData);
        if (isset($options['autoUpdate']) && $options['autoUpdate']) {
            $this->generateSetting('w:updateFields');
        }
        PhpdocxLogger::logger('Add index to word document.', 'info');
        if ($class == 'WordFragment') {
            $this->wordML .= (string)$index;
        } else {
            $this->_wordDocumentC .= (string)$index;
        }
    }
     ...
     * @throws Exception method not available, image does not exist
     */
    public function addLineSignature($options = array())
    {
        if (!file_exists(__DIR__ . '/CreateLineSignature.php')) {
            PhpdocxLogger::logger('This method is not available for your license.', 'fatal');
        }
        ...
        if ($options['target'] == 'defaultHeader' ||
            ...
            CreateDocx::$_relsHeaderFooterImage[$options['target']][] = array('rId' => 'rId' . self::$intIdWord, 'extension' => $imageExtension);
        } else if ($options['target'] == 'footnote' ||
                $options['target'] == 'endnote' ||
                $options['target'] == 'comment') {
            CreateDocx::$_relsNotesImage[$options['target']][] = array('rId' => 'rId' . self::$intIdWord, 'extension' => $imageExtension);
        } else {
            $this->generateRELATIONSHIP('rId' . self::$intIdWord, 'image', 'media/imgrId' . self::$intIdWord . '.' . $imageExtension);
        }
    ...
    public function addMathEquation($equation, $type, $options = array())
    {
        ...
        if ($type == 'docx') {
            ...
            if ($class == 'WordFragment') {
                $this->wordML .= '<w:p><m:oMathPara>' . $eqs[1] . '</m:oMathPara></w:p>';
            } else {
                $this->_wordDocumentC .= '<w:p><m:oMathPara>' . $stylesMathEq . $eqs[1] . '</m:oMathPara></w:p>';
            }
            $package->close();
        } elseif ($type == 'omml') {
            $class = get_class($this);
            ...
            if ($class == 'WordFragment') {
                $this->wordML .= '<w:p>' . (string)$equation . '</w:p>';
            } else {
                $this->_wordDocumentC .= '<w:p>' . (string)$equation . '</w:p>';
            }
        } elseif ($type == 'mathml') {
            $class = get_class($this);
            ...
            if ($class == 'WordFragment') {
                $this->wordML .= '<w:p><m:oMathPara>' . $stylesMathEq . (string)$math . '</m:oMathPara></w:p>';
            } else {
                $this->_wordDocumentC .= '<w:p><m:oMathPara>' . $stylesMathEq . (string)$math . '</m:oMathPara></w:p>';
            }
        }
    }
    ...
    {
        if (!file_exists(__DIR__ . '/DOCXPath.php')) {
            PhpdocxLogger::logger('This method is not available for your license.', 'fatal');
        }
        ...
        if (!isset($options['image'])) {
            $options['image'] = __DIR__ . '/../templates/black.png';
        }
    ...
    {
        if (!file_exists(__DIR__ . '/CryptoPHPDOCX.php')) {
            PhpdocxLogger::logger('This method is not available for your license.', 'fatal');
        }
    ...
    {
        if (!file_exists(__DIR__ . '/CreateSource.php')) {
            PhpdocxLogger::logger('This method is not available for your license.', 'fatal');
        }
     ...
     *
     * @access public
     * @param string $type checkbox, comboBox, date, dropDownList, richText, text
     * @param array $options Style options to apply to the text
     *  Values:
     ...
     * 'wordFragment' (WordFragment) richText type. Add WordFragment content without adding a placeholder
     * For other options @see addText
    ...
    public function addStructuredDocumentTag($type, $options = array())
    {
        ...
        if (isset($options['wordFragment']) && $options['wordFragment'] instanceof WordFragment && $type == 'richText') {
     ...
     * @throws Exception method not available
     */
    public function addTableAuthorities($options = array(), $legend = array())
    {
        if (!file_exists(__DIR__ . '/CreateTableAuthorities.php')) {
            PhpdocxLogger::logger('This method is not available for your license.', 'fatal');
        }
        ...
        $legend = self::setRTLOptions($legend);
        if (!isset($legend['text']) || empty($legend['text'])) {
            $legend['text'] = 'No table of authorities entries found.';
        }
        $legendOptions = $legend;
        unset($legendOptions['text']);
        $class = get_class($this);
        $legendData = new WordFragment();
        $legendData->addText(array($legend), $legendOptions);
        $tableAuthorities = CreateTableAuthorities::getInstance();
        $tableAuthorities->createTableAuthorities($options, $legendData);
        if (isset($options['autoUpdate']) && $options['autoUpdate']) {
            $this->generateSetting('w:updateFields');
        }
        PhpdocxLogger::logger('Add table of authorities to word document.', 'info');
        if ($class == 'WordFragment') {
            $this->wordML .= (string) $tableAuthorities;
        } else {
            $this->_wordDocumentC .= (string) $tableAuthorities;
        }
    }
    ...
    public function createDocx($fileName = 'document')
    {
        ...
        if (file_exists(__DIR__ . '/RepairPDF.php') && is_array($this->_repairMode)) {
     ...
     * @param array $referenceNode
     * Keys and values:
     *     'target' (string) document (default), style, lastSection, header, footer, numbering, theme
     *     'type' (string) break, image, list, math, numbering, paragraph, run, section, style, table, table-row, table-cell, table-cell-paragraph
     ...
     *     'width' (int) in twentieths of a point
     * Values theme:
     *     'color' (ffffff, ff0000...) accent1, accent2, accent3, accent4, accent5, accent6, dk1, dk2, folHlink, hlink, lt1, lt2
     * Common values:
    ...
    {
        if (!file_exists(__DIR__ . '/DOCXCustomizer.php')) {
            PhpdocxLogger::logger('This method is not available for your license.', 'fatal');
        }
        ...
        } else if ($referenceNode['target'] == 'theme')  {
            // theme target
            $options['tagType'] = 'theme';
            $themeXML = $this->_zipDocx->getContent('word/theme/theme1.xml');

            if ($themeXML !== false) {
                $domDocument = $this->xmlUtilities->generateDomDocument($themeXML, true);
                $domXpath = new DOMXPath($domDocument);
                ...
                if ($contentNodes->length > 0) {
                    ...
                    $this->_zipDocx->addContent('word/theme/theme1.xml', $domDocument->saveXML());
                }
            }
        } else {
            // default as document
     ...
     * 'isFile' (bool) true for URLs and false for strings. Default as false
     * 'parseAnchors' (bool)
    ...
    public function embedHTML($html = '<html><body></body></html>', $options = array())
    {
        ...
        if (!extension_loaded('tidy') && !$options['forceNotTidy']) {
            throw new Exception('Install and enable Tidy for PHP (https://php.net/manual/en/book.tidy.php) to transform HTML to DOCX. If PHP Tidy can\'t be installed, enable the forceNotTidy option.');
        }
        ...
        if (file_exists(__DIR__ . '/HTMLExtended.php') && $options['useHTMLExtended'] && strpos($html, '<math') !== false) {
        ...
        if (is_array($sFinalDocX[4])) {
            foreach ($sFinalDocX[4] as $value) {
                ...
                if (file_exists(__DIR__ . '/HTMLExtended.php') && $options['useHTMLExtended']) {
        ...
        if (file_exists(__DIR__ . '/HTMLExtended.php') && $options['useHTMLExtended'] && count($nodesMathEquation) > 0) {
     ...
     * @param array $referenceNode (if empty or null force append)
     * Keys and values:
     *     'type' (string) can be * (all, default value), bookmark, break, chart, endnote (content reference), footnote (content reference), image, list, math, paragraph (also for links and lists), section, shape, table, table-row, table-cell, table-cell-paragraph
     *     'contains' (string) for bookmark, list, paragraph (text, link), shape
     *     'occurrence' (int) exact occurrence, (array) occurrences, (string) range of contents (e.g.: 2..9, 2.., ..9) or first() or last(), if empty iterate all elements
     *     'attributes' (array)
     *     'parent' (string) w:body (default), '/' (any parent) or any other specific parent (/w:tbl/, /w:tc/, /w:r/...)
     ...
     * @throws Exception method not available
     */
    public function getWordContents($referenceNode, $options = array())
    {
        if (!file_exists(__DIR__ . '/DOCXPath.php')) {
            PhpdocxLogger::logger('This method is not available for your license.', 'fatal');
        }
        ...
        // default options
        if (!isset($options['parseAsParagraphs'])) {
            $options['parseAsParagraphs'] = false;
        }
        ...
        if ($referenceNode['target'] == 'header' || $referenceNode['target'] == 'footer') {
            ...
            if (count($contents) > 0) {
                ...
                foreach ($contents as $content) {
                    ...
                    foreach ($contentNodes as $contentNode) {
                        if (isset($options['getAsParagraphs']) && $options['getAsParagraphs']) {
                            // get as paragraphs
                            if ($contentNode->nodeName == 'w:p') {
                                // if the content node is a paragraph, add it as a single entry
                                $paragraphEntries = array($contentNode);
                            } else {
                                // get paragraphs from the content node
                                $paragraphEntries = $contentNode->getElementsByTagNameNS('http://schemas.openxmlformats.org/wordprocessingml/2006/main', 'p');
                            }
                            $content = '';
                            foreach ($paragraphEntries as $paragraphEntry) {
                                // if empty paragraph avoid adding the content
                                if (empty($paragraphEntry->textContent) || !($paragraphEntry instanceof DOMElement)) {
                                    continue;
                                }

                                $textEntries = $paragraphEntry->getElementsByTagNameNS('http://schemas.openxmlformats.org/wordprocessingml/2006/main', 't');
                                foreach ($textEntries as $textEntry) {
                                    $content .= $textEntry->textContent;
                                }
                                $content .= ' ';
                            }
                            $contentNodesTextValues[] = $content;
                        } else {
                            // parse as text nodes
     ...
     * @param array $referenceNode (if empty or null force append)
     * Keys and values:
     *     'type' (string) can be chart, image, default, list, math, paragraph (also for links and lists), run, style, table, table-row, table-cell, table-cell-paragraph or a custom tag
     *     'contains' (string) for bookmark, list, paragraph (text, link), shape
     *     'occurrence' (int) exact occurrence, (array) occurrences, (string) range of contents (e.g.: 2..9, 2.., ..9) or first() or last(), if empty iterate all elements
     *     'attributes' (array)
     *     'parent' (string) w:body (default), '/' (any parent) or any other specific parent (/w:tbl/, /w:tc/, /w:r/...)
    ...
    /**
     * Imports headers and/or footers from a Word file
     *
     ...
     *
     * @access public
     * @param string|DOCXStructure $document path or DOCXStructure of the document that includes the contents to be imported
     * @param array $referenceNode
     * Keys and values:
     *     'type' (string) can be * (all, default value), bookmark, break, chart, endnote (content reference), footnote (content reference), image, list, math, paragraph (also for links and lists), section, shape, table, table-row, table-cell, table-cell-paragraph
     *     'contains' (string) for bookmark, list, paragraph (text, link), shape
    ...
    {
        if (file_exists(__DIR__ . '/DOCXStructureTemplate.php') && $document instanceof DOCXStructure) {
            // use in-memory DOCX
        ...
        $documentStructure = $merge->mergeDocx($this->createDocx(''), array($documentInMemory), 'output.docx');
    ...
    /**
     * Imports theme from a Word file
     *
     * @param string|DOCXStructure $document Path to the DOCX or DOCXStructure to import the theme
     * @throws Exception method not available, error opening the file
     */
    public function importTheme($document)
    {
        if (!file_exists(__DIR__ . '/Themes.php')) {
            PhpdocxLogger::logger('This method is not available for your license.', 'fatal');
        }

        if ($document instanceof DOCXStructure) {
            $documentTheme = $document;
        } else {
            $documentTheme = new DOCXStructure();
            $documentTheme->parseDocx($document);
        }

        $newThemeXML = $documentTheme->getContent('word/theme/theme1.xml');

        if ($newThemeXML !== false) {
            $this->_zipDocx->addContent('word/theme/theme1.xml', $newThemeXML);

            PhpdocxLogger::logger('Import theme.', 'info');
        }
    }

    /**
     * Inserts a WordFragment before or after a node into the document content
     *
     * @access public
     * @param WordFragment $wordFragment the WordFragment to insert
     * @param array $referenceNode (if empty or null force append)
     * Keys and values:
     *     'type' (string) can be * (all, default value), bookmark, break, chart, endnote (content reference), footnote (content reference), image, list, math, paragraph (also for links and lists), section, shape, table, table-row, table-cell, table-cell-paragraph
     *     'contains' (string) for bookmark, list, paragraph (text, link), shape
     ...
     *      'subEntry' (string)
     *      'type' (string) 'cross-reference', 'current-page' (default), 'page-range'
     *      'seeText' (string) used in cross-reference entries. Default as 'See'
     *      'bookmark' (string) required in page-range entries
     * @param array $styles rPr styles @see createCharacterStyle
     * @throws Exception method not available, not used as WordFragment, bookmark required for page-range entries
     */
    public function markEntry($options = array(), $styles = array())
    {
        if (!file_exists(__DIR__ . '/CreateIndex.php')) {
            PhpdocxLogger::logger('This method is not available for your license.', 'fatal');
        }

        if (!isset($options['mainEntry'])) {
            $options['mainEntry'] = 'Entry';
        }
        $options['mainEntry'] = $this->parseAndCleanTextString($options['mainEntry']);
        if (!isset($options['subEntry'])) {
            $options['subEntry'] = '';
        } else {
            $options['subEntry'] = ':' . $options['subEntry'];
        }
        $options['subEntry'] = $this->parseAndCleanTextString($options['subEntry']);
        if (!isset($options['type'])) {
            $options['type'] = 'current-page';
        }
        if (!isset($options['seeText'])) {
            $options['seeText'] = 'See';
        }
        $options['seeText'] = $this->parseAndCleanTextString($options['seeText']);

        if ($options['type'] == 'page-range' && !isset($options['bookmark'])) {
            PhpdocxLogger::logger('A bookmark is required for page-range entries.', 'fatal');
        }

        $rPrStyles = '';
        if (count($styles) > 0) {
            $stylesMarkEntry = new CreateParagraphStyle();
            $styles = self::translateTextOptions2StandardFormat($styles);
            $stylesMarkEntry->createCustomCharacterStyle('entrystyle', $styles);
            $rPrStyles = $stylesMarkEntry->XMLRPr();
        }

        $markEntry = '<w:r><w:fldChar w:fldCharType="begin"/></w:r>';
        $markEntry .= '<w:r>'.$rPrStyles.'<w:instrText xml:space="preserve"> XE &quot;</w:instrText></w:r>';
        $markEntry .= '<w:r>'.$rPrStyles.'<w:instrText xml:space="preserve">'.$options['mainEntry'].' </w:instrText></w:r>';
        if (isset($options['subEntry']) && !empty($options['subEntry'])) {
            $markEntry .= '<w:r>'.$rPrStyles.'<w:instrText xml:space="preserve">'.$options['subEntry'].' </w:instrText></w:r>';
        }
        if ($options['type'] == 'cross-reference') {
            $markEntry .= '<w:r><w:instrText>&quot; \t &quot;</w:instrText></w:r>';
            $markEntry .= '<w:r><w:instrText xml:space="preserve">' . $options['seeText'] . '</w:instrText></w:r>';
        } else if ($options['type'] == 'page-range') {
            $markEntry .= '<w:r><w:instrText>&quot; \r &quot;</w:instrText></w:r>';
            $markEntry .= '<w:r><w:instrText>' . $options['bookmark'] . '</w:instrText></w:r>';
        }
        $markEntry .= '<w:r><w:instrText xml:space="preserve">&quot; </w:instrText></w:r>';
        $markEntry .= '<w:r><w:fldChar w:fldCharType="end"/></w:r>';

        PhpdocxLogger::logger('Mark entry.', 'info');

        $class = get_class($this);
        if ($class == 'WordFragment') {
            $this->wordML .= $markEntry;
        } else {
            PhpdocxLogger::logger('This method must be used as WordFragment.', 'fatal');
        }
    }
     ...
     * @param array $referenceNode
     * Keys and values:
     *     'type' (string) can be * (all, default value), break, chart, endnote (content reference, the whole paragraph is removed), footnote (content reference, the whole paragraph is removed), image, list, math, paragraph (also for bookmarks, links and lists), section, shape, table, table-row, table-cell, table-cell-paragraph
     *     'contains' (string) for list, paragraph (text, bookmark, link), shape
     *     'occurrence' (int) exact occurrence, (array) occurrences, (string) range of contents (e.g.: 2..9, 2.., ..9) or first() or last(), if empty iterate all elements
     *     'attributes' (array)
     *     'parent' (string) w:body (default), '/' (any parent) or any other specific parent (/w:tbl/, /w:tc/, /w:r/...)
    ...
    /**
     * Replaces a Word content by a WordFragment
     *
     * @access public
     * @param WordFragment $wordFragment the WordFragment to insert
     * @param array $referenceNode
     * Keys and values:
     *     'type' (string) can be * (all, default value), break, chart, endnote (content reference), footnote (content reference), image, math, paragraph (also for bookmarks, links and lists), section, shape, table, table-row, table-cell, table-cell-paragraph
     *     'contains' (string) for list, paragraph (text, bookmark, link), shape
     *     'occurrence' (int) exact occurrence, (array) occurrences, (string) range of contents (e.g.: 2..9, 2.., ..9) or first() or last(), if empty iterate all elements
     *     'attributes' (array)
     *     'parent' (string) w:body (default), '/' (any parent) or any other specific parent (/w:tbl/, /w:tc/, /w:r/...)
     ...
     *   'path' (string) set the path to LibreOffice. If set, overwrite the path setting in phpdocxconfig.ini
    ...
    {
        if (file_exists(__DIR__ . '/TransformDocAdv.php')) {
    ...
    public function transformOMMLToMathML($omml, $options = array()) {
        ...
        $objXSL->load(__DIR__ . '/../xsl/OMML2MML.XSL');
        ...
        if (isset($options['cleanNamespaces']) && $options['cleanNamespaces'] == false) {
            ...
            // remove namespaces
            $mathML = str_replace(array('mml:', 'xmlns:'), '', $mathML);
            // set the standard xmlns
            $mathML = str_replace('<math mml="http://www.w3.org/1998/Math/MathML" m="http://schemas.openxmlformats.org/officeDocument/2006/math">', '<math xmlns="http://www.w3.org/1998/Math/MathML">', $mathML);
     ...
     *
     * @access protected
     * @param mixed $wordFragment the WordFragment that we wish to insert
    ...
    protected function applyMathEquationStyles($equation, $options)
    {
        ...
        return $themeMath->theme($equation, $options);
    }
                ...
                if (count($indexIds) > 0) {
                    ...
                    foreach ($indexIds as $indexId) {
                        ...
                        if ($elementsRelationship->length > 0) {
                            ...
                            // add the target only if it has not been added before
                            $newTarget = true;
                            foreach ($contents as $content) {
                                if ($content['target'] == 'word/' . $elementsRelationship->item(0)->getAttribute('Target')) {
                                    $newTarget = false;
                                    break;
                                }
                            }
     ...
     * @param array $referenceNode
     * Keys and values:
     *     'type' (string) can be * (all, default value), break, chart, endnote (content reference), footnote (content reference), image, list, math, paragraph (also for bookmarks, links and lists), section, shape, table, table-row, table-cell, table-cell-paragraph, tracking-insert, tracking-delete, tracking-run-style, tracking-paragraph-style, tracking-table-style, tracking-table-grid, tracking-table-row
     *     'contains' (string) for list, paragraph (text, bookmark, link), shape
     *     'occurrence' (int) exact occurrence, (array) occurrences, (string) range of contents (e.g.: 2..9, 2.., ..9) or first() or last(), if empty iterate all elements
     *     'attributes' (array)
     *     'parent' (string) w:body (default), '/' (any parent) or any other specific parent (/w:tbl/, /w:tc/, /w:r/...)
    ...
    /**
     * Insert a WordFragment before a certain node
     *
     * @access protected
     * @param mixed $wordFragment the WordFragment that we wish to insert

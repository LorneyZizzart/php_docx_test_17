 ...
 */
if (!defined('PARSERHTML_DIR')) {
  define("PARSERHTML_DIR", str_replace(DIRECTORY_SEPARATOR, '/', __DIR__));
}

/**
 * The location of the DOMPDF include directory
 */
if (!defined('PARSERHTML_INC_DIR')) {
...
class FrameParser_Tree {
  ...
  protected function _build_tree_r(DOMNode $node) {
    if (file_exists(__DIR__ . '/HTMLExtended.php') && HTML2WordML::$htmlExtended) {
...
class PARSERHTML {
  ...
  protected function _process_html($filter = '*') {
    ...
    if (CreateDocx::$baseCSSHTML != null && file_exists(__DIR__ . '/HTMLExtended.php')) {
  ...
  private function getAttributes($node){
    ...
    switch ($node->nodeName) {
      ...
      case 'img':
        ...
        if ($this->baseURL == '' && !is_file(urldecode($aRet['src']))) {
...
class StyleParser {
  ...
  function munge_colour($colour) {
    ...
    // #rgba format
    } else if ( $length == 5 && $colour[0] === "#" ) {
      $alphaValue = round(hexdec($colour[4] . $colour[4]) / 255, 2);
      return $this->getArray($colour[1].$colour[1].$colour[2].$colour[2].$colour[3].$colour[3], $alphaValue);
    ...
    // #rrggbbaa format
      ...
      $alphaValue = round(hexdec($colour[7] . $colour[8]) / 255, 2);
      return $this->getArray(mb_substr($colour, 1, 6), $alphaValue);
    ...
    // rgb( r,g,b ) and rgba( r,g,b,a ) formats
      ...
      return $this->getArray(vsprintf("%02X%02X%02X", $triplet), $alpha);
    ...
    // hsl( h,s,l ) and hsla( h,s,l,a ) formats
      ...
      return $this->getArray(vsprintf("%02X%02X%02X", $tripletRGB), $alpha);
    // cmyk( c,m,y,k ) format
  ...
  private function getArray($colour, $alpha = null) {
    ...
    if (is_array($colour)) {
      ...
      $c["b"] = $c[2];*/
      if (!is_null($alpha)) {
        list($rColour, $gColour, $bColour) = sscanf($colour, '%02x%02x%02x');
        $colour = array(round($rColour * $alpha + 255 * (1 - $alpha)), round($gColour * $alpha + 255 * (1 - $alpha)), round($bColour * $alpha + 255 * (1 - $alpha)));
        $colour = sprintf('%02x%02x%02x', $colour[0], $colour[1], $colour[2]);
      }
      $c["hex"] = "#$colour";
    }
 ...
 */
if (!defined('__DEFAULT_STYLESHEET')) {
  define('__DEFAULT_STYLESHEET', __DIR__ . '/../templates/html.css');
}

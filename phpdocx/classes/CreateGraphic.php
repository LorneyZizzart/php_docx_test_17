...
class CreateGraphic extends CreateElement
{
    ...
    public function createEmbeddedGraphic()
    {
        ...
        if ($this->_type != '') {
            ...
            PhpdocxLogger::logger('Missing chart type', 'fatal');
            return false;
        }
    }
    ...
    public function CreateGraphic()
    {
        ...
        if (isset($this->_type) && isset($args[0])) {
            if ($this->createEmbeddedXmlChart() == false) {
                PhpdocxLogger::logger('Missing legends', 'fatal');
                return false;
            }
    ...
    protected function addComboChart($id)
    {
        ...
        $domChart = $xmlUtilities->generateDomDocument($this->_comboChart);
    ...
    {
        $xml = '<' . CreateGraphic::NAMESPACEWORD . ':title>__PHX=__GENERATETITLE__';

        if ($this->_stylesTitle !== null && is_array($this->_stylesTitle) && isset($this->_stylesTitle['layout'])) {
            $xml .= '<c:layout><c:manualLayout><c:xMode val="edge"/><c:yMode val="edge"/><c:x val="'.$this->_stylesTitle['layout']['x'].'"/><c:y val="'.$this->_stylesTitle['layout']['y'].'"/></c:manualLayout></c:layout>';
        }

        $xml .= '<c:overlay val="0" /></' . //We include the overlay=0 as a hack for Word 2010
     ...
     * @access protected
     */
    protected function generateMARKER($symbol = 'none', $size = null, $options = null)
    {
        ...
        if (isset($options['color']) || isset($options['borderColor'])) {
            $xml .= '<c:spPr>';
            if (isset($options['color'])) {
                $xml .= '<a:solidFill><a:srgbClr val="' . $options['color'] . '"/></a:solidFill>';
            }
            if (isset($options['borderColor'])) {
                $xml .= '<a:ln w="9525"><a:solidFill><a:srgbClr val="' . $options['borderColor'] . '"/></a:solidFill></a:ln>';
            }
            $xml .= '</c:spPr>';
        }

...
class CreateLineChart extends CreateGraphic implements InterfaceGraphic
{
    ...
    public function createEmbeddedXmlChart()
    {
        ...
        if (isset($this->values['legend'])) {
            ...
            PhpdocxLogger::logger('Legends are missing.', 'fatal');
            return false;
        }
        ...
        for ($i = 0; $i < count($legends); $i++) {
            ...
            if (!empty($this->_symbol)) {
                ...
                if (isset($this->_theme['serMarkerOptions']) && isset($this->_theme['serMarkerOptions'][$i])) {
                    $markerOptions = $this->_theme['serMarkerOptions'][$i];
                }
                if (is_array($this->_symbol)) {
                    $this->generateMARKER($this->_symbol[$i], $sizeSimbol, $markerOptions);
                } else {
                    $this->generateMARKER($this->_symbol, $sizeSimbol, $markerOptions);
                }
            }
            $this->cleanTemplate2();
